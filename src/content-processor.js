...
class ContentProcessor {
  ...
  postProcessHtml(htmlContent) {
    // Add IDs to headers for PDF anchor links
    htmlContent = this.addHeaderIds(htmlContent);

    // Process two-column layout markers
    htmlContent = this.processTwoColumnLayout(htmlContent);
  ...
  addHeaderIds(htmlContent) {
    // Add IDs to headers for PDF anchor links
    // Converts <h1>Title</h1> to <h1 id="title">Title</h1>
    const headerRegex = /<h([1-6])>([^<]+)<\/h\1>/g;

    return htmlContent.replace(headerRegex, (match, level, text) => {
      // Generate ID from header text: lowercase, replace spaces with hyphens, remove special chars
      const id = text
        .toLowerCase()
        .trim()
  ...
  processTwoColumnLayout(htmlContent) {
    // Process multi-column layout markers
    // Look for <!-- two-columns -->, <!-- three-columns -->, etc.
...
.${columnCount}-column-layout li {
  margin: 0.25em 0;
    ...
    // Look for <!-- column-break --> comments
    htmlContent = htmlContent.replace(
      /<!--\s*column-break\s*-->/gi,
      '<div style="break-inside: avoid; column-break-after: always;"></div>'
    );
  ...
  processAnchorLinks(htmlContent) {
    // Process anchor links to ensure they work in PDFs
    // Convert markdown-style anchor links (#section-name) to work with header IDs

    // First, collect all header IDs that were generated by marked.js
    const headerIdRegex = /<h[1-6]\s+id="([^"]+)"[^>]*>([^<]+)<\/h[1-6]>/g;
    const headerIds = new Set();
    let headerMatch;

    while ((headerMatch = headerIdRegex.exec(htmlContent)) !== null) {
      headerIds.add(headerMatch[1]);
    }

    // Now process anchor links to ensure they reference valid header IDs
    // Convert links like <a href="#section-name"> to ensure they match header IDs
    let result = htmlContent;

    // Fix anchor links that might have different formatting
    result = result.replace(/<a\s+href="#([^"]+)"([^>]*)>([^<]+)<\/a>/g, (match, href, attrs, text) => {
      // The href should match a header ID
      // marked.js converts headers to lowercase with hyphens
      const normalizedHref = href.toLowerCase().replace(/\s+/g, "-");

      // Check if this ID exists in our headers
      if (headerIds.has(normalizedHref) || headerIds.has(href)) {
        // Link is valid, keep it as is
        return match;
      } else {
        // Try to find a matching header by normalizing the text
        const normalizedText = text.toLowerCase().replace(/\s+/g, "-");
        if (headerIds.has(normalizedText)) {
          return `<a href="#${normalizedText}"${attrs}>${text}</a>`;
        }
      }
    ...
    console.log(chalk.blue(`🔗 Processed anchor links for PDF navigation`));
    return result;
  }
